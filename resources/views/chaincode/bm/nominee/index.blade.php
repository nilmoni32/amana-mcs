        ...
        <div class="tile">
            <div class="tile-body">
                <div class="table-scrollbar">
                    <table class="table table-hover table-bordered" id="BmNomineeTable">
                        <thead>
                            <tr>
                                <th> Sl </th>
                                <th> Name </th>
                        ...
                        <tbody>
                            @foreach($bmNominees as $bmNominee)
                            <tr>
                                ...
                                <td style="padding: 0.5rem; vertical-align: 0 ;">
                                    {{ $bmNominee->name }}</td> 
                                <td style="padding: 0.5rem; vertical-align: 0 ;">
                                    {{ $bmNominee->father_name }}</td> 
                                <td style="padding: 0.5rem; vertical-align: 0 ;">
                                    {{ $bmNominee->husband_name }}</td> 
                                <td style="padding: 0.5rem; vertical-align: 0 ;">
                                    {{ $bmNominee->relation }}</td>
                                <td style="padding: 0.5rem; vertical-align: 0 ;">
                                    {{ $bmNominee->relation_percentage }}</td>                               
                                <td class="text-center" style="vertical-align: 0 ;">
                                    <div class="btn-group" role="group" aria-label="Second group">
                                        <a href="{{ route('BMcode.nominee.show', $bmNominee->id) }}" 
                                            title='SHOW' class='btn btn-sm btn-primary text-light'><i class='fa fa-user mr-1'></i></a>
                                        <a href="{{ route('BMcode.nominee.edit', $bmNominee->id) }}"
                                            title='EDIT' class="btn btn-sm btn-warning"><i class="fa fa-edit"></i></a>
                                        <a href="{{ route('BMcode.nominee.delete', $bmNominee->id) }}"
