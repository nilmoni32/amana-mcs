        ...
        <div class="tile">
            <div class="tile-body">
                <div class="table-scrollbar">
                    <table class="table table-hover table-bordered" id="RsmNomineeTable">
                        <thead>
                            <tr>
                                <th> Sl </th>
                                <th> Name </th>
                        ...
                        <tbody>
                            @foreach($rsmNominees as $rsmNominee)
                            <tr>
                                ...
                                <td style="padding: 0.5rem; vertical-align: 0 ;">
                                    {{ $rsmNominee->name }}</td> 
                                <td style="padding: 0.5rem; vertical-align: 0 ;">
                                    {{ $rsmNominee->father_name }}</td> 
                                <td style="padding: 0.5rem; vertical-align: 0 ;">
                                    {{ $rsmNominee->husband_name }}</td> 
                                <td style="padding: 0.5rem; vertical-align: 0 ;">
                                    {{ $rsmNominee->relation }}</td>
                                <td style="padding: 0.5rem; vertical-align: 0 ;">
                                    {{ $rsmNominee->relation_percentage }}</td>                               
                                <td class="text-center" style="vertical-align: 0 ;">
                                    <div class="btn-group" role="group" aria-label="Second group">
                                        <a href="{{ route('RSMcode.nominee.show', $rsmNominee->id) }}" 
                                            title='SHOW' class='btn btn-sm btn-primary text-light'><i class='fa fa-user mr-1'></i></a>
                                        <a href="{{ route('RSMcode.nominee.edit', $rsmNominee->id) }}"
                                            title='EDIT' class="btn btn-sm btn-warning"><i class="fa fa-edit"></i></a>
                                        <a href="{{ route('RSMcode.nominee.delete', $rsmNominee->id) }}"
